 ...
 */
public class LocationByTest extends Service implements LocationListener {

    private static final String TAG = "LocationSvc";
    private LocationManager locationManager;
    public static final int time = 60*1;
...
//        if (MainTabIndexFragment.list.size()>0){
                    ...
                    List<MyLockEntity> myLock = getMyLock();
                    ...
                    if (myLock == null) {
                        ...
                        bundle.putString(Config.ISCLOSE,"true");
                        ...
                        Log.e("TAG","----------++==>>>>myLock.toString()=="+myLock.toString());
                        for (int i = 0; i < myLock.size(); i++) {
                            //经纬不全  或者未入住不处理。
                            if (myLock.get(i).getLatitude().equals("null") || myLock.get(i).getLongitude().equals("null")|| myLock.get(i).getCheckin_time().equals("null")) {
                                //继续遍历下一个酒店
                            } else {
                                double lad2 = Double.parseDouble(myLock.get(i).getLatitude()) / 1000000;
                                double lng2 = Double.parseDouble(myLock.get(i).getLongitude()) / 1000000;
                                double m = DistanceOfTwoPoints(latitude, longitude, lad2, lng2);
                                Log.e("TAG","locationbytest-------------->"+m);
//                    AppContext.toast("m="+m);
                                if (m >500) {
                                    //通知Activity
                                    if (AppContext.getProperty(myLock.get(i).getBook_id()) == null || !AppContext.getProperty(myLock.get(i).getBook_id()).equals("0")) {
                                        AppContext.setProperty(myLock.get(i).getBook_id(), "0");
                                        if(i!=myLock.size()-1){
                                            try {
...
//                            AppContext.toast(m+"");
                                        ...
                                        bundle.putString(Config.MYLOCKBOOKID,myLock.get(i).getBook_id());
                                        bundle.putString(Config.MYLOCKBOOKID_ROOMID,myLock.get(i).getRoom_id());
                                        bundle.putString(Config.MYHOTEL_CAPTION,myLock.get(i).getHotel_caption());
                                        intentLock.putExtras(bundle);
                                        sendBroadcast(intentLock);
                                    }
                                    // 如果只是需要定位一次，这里就移除监听，停掉服务。如果要进行实时定位，可以在退出应用或者其他时刻停掉定位服务。
//                locationManager.removeUpdates(this);
                                    ...
                                    if (AppContext.getProperty(myLock.get(i).getBook_id()) == null || !AppContext.getProperty(myLock.get(i).getBook_id()).equals("1")) {
                                        AppContext.setProperty(myLock.get(i).getBook_id(), "1");
//                            AppContext.toast(m + "");
                                        Intent intentLock = new Intent();
                                        intentLock.setAction(Common.LOCATION_ACTION_BYPOWER);
                                        intentLock.putExtra(Common.LOCATION, location.toString());
...
//                    stopSelf();
        ...
        } else if (locationManager.getProvider(LocationManager.NETWORK_PROVIDER) != null) locationManager
...
//            double lng2 = Double.parseDouble(AppContext.getProperty(Config.CHINCKIN_LON)) / 1000000;
            ...
            if (m > 5) {
                if (AppContext.getProperty(Config.CLOSEPOWER) != null) {//退房后关闭服务
                    stopSelf();
                    // 如果只是需要定位一次，这里就移除监听，停掉服务。如果要进行实时定位，可以在退出应用或者其他时刻停掉定位服务。
                    locationManager.removeUpdates(LocationByTest.this);
                } else {
                    //通知Activity
                    Intent intent = new Intent();
                    intent.setAction(Common.LOCATION_ACTION_BYPOWER);
...
//                locationManager.removeUpdates(this);
    ...
    private List<MyLockEntity> getMyLock() {
        if (AppContext.getProperty(Config.MYLOCKORDER) != null) {
            JSONObject object = null;
//            List<MyLockEntity> list=new ArrayList<>();
            try {
                object = new JSONObject(AppContext.getProperty(Config.MYLOCKORDER));
                String result = object.getString("result");
                if (result.equals("false")) {//没有入住
                    return null;
                } else {
                    ResponseBean responseBean = new ResponseBean(AppContext.getProperty(Config.MYLOCKORDER));
                    List<MyLockEntity> list_s = responseBean.getListData(MyLockEntity.class);
//                    Log.e("TAG", "------===-------------->" + responseBean.toString() + "====list_s.toString" + list_s.toString());
                    return list_s;
                }
