 ...
 */
package com.sht.smartlock.ui.chat.applib.adapter;
...
import com.sht.smartlock.ui.chat.applib.activity.ContextMenu;
import com.sht.smartlock.ui.chat.applib.activity.MyAlertDialog;
import com.sht.smartlock.ui.chat.applib.activity.ShowBigImage;
import com.sht.smartlock.ui.chat.applib.activity.ShowNormalFileActivity;
import com.sht.smartlock.ui.chat.applib.activity.ShowVideoActivity;
import com.sht.smartlock.ui.chat.applib.activity.UserProfileActivity;
import com.sht.smartlock.ui.chat.applib.controller.HXSDKHelper;
import com.sht.smartlock.ui.chat.applib.domain.User;
import com.sht.smartlock.ui.chat.applib.task.LoadImageTask;
import com.sht.smartlock.ui.chat.applib.task.LoadVideoImageTask;
import com.sht.smartlock.ui.chat.applib.uidemo.Constant;
import com.sht.smartlock.ui.chat.applib.uidemo.DemoHXSDKHelper;
import com.sht.smartlock.ui.chat.applib.utils.DateUtils;
import com.sht.smartlock.ui.chat.applib.utils.ImageCache;
...
public class MessageAdapter extends BaseAdapter{
	...
	public MessageAdapter(Context context, String username, int chatType,List<User> myUsers) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if ((chatType == ChatType.GroupChat || chatType == ChatType.ChatRoom) && message.direct == Direct.RECEIVE){
			...
			String str_waiterJson = AppContext.getProperty(Config.WAITERJSON);
			List<String> list = new ArrayList<>();
			if(str_waiterJson != null){
				try{
					JSONObject jsonObject = new JSONObject(str_waiterJson);
					JSONArray jsonArray = jsonObject.getJSONArray("result");
					for(int i = 0;i < jsonArray.length();i ++){
						list.add(jsonArray.getJSONObject(i).getString("emid"));
					}
//					list.add("33ispj7u6572");
					LogUtil.log("adas=" + list.get(0));
				}catch (Exception e){
                LogUtil.log(e.toString());
				}
			}

			boolean contains = list.contains(message.getFrom());
			if(contains){
				holder.tv_usernick.setText("本店店小二");
			}else{
				UserUtils.setUserNick(message.getFrom(), holder.tv_usernick);
			}
//			if(message.getUserName().equals(str_waiterJson)
		}
	...
	private void setUserAvatar(final EMMessage message, ImageView imageView){
	    if(message.direct == Direct.SEND){
	        //显示自己头像
			imageView.setTag(message.getFrom());
	        UserUtils.setCurrentUserAvatar(context, imageView);
			if (AppContext.getProperty(Config.MyImg)!=null){
				ImageLoader.getInstance().displayImage(AppContext.getProperty(Config.MyImg),imageView,displayOptions);
			}
	    }else{
	        UserUtils.setUserAvatar(context, message.getFrom(), imageView);
			for (int i=0;i<myUsers.size();i++){
				if ( message.getFrom().equals(myUsers.get(i).getEmid())){
			        ImageLoader.getInstance().displayImage(myUsers.get(i).getId_image(),imageView,displayOptions);
//					UserUtils.getUserInfo( message.getFrom()).setAvatar(myUsers.get(i).getId_image());
//					UserUtils.setUserAvatar(context, message.getFrom(), imageView);
				}
			}
	    }
