...
//			    holder.unreadMsgView.setText(user.getUnreadMsgCount()+"");
		    ...
		    holder.nameTextview.setText(user.getNick());
		    //设置用户头像
			if (userList.get(position).getId_image() != null && ! userList.get(position).getId_image().equals("null")){
				ImageLoader.getInstance().displayImage(userList.get(position).getId_image(),holder.avatar);
				//设置用户的头像
//				UserUtils.getUserInfo(userList.get(position).getEmid()).setAvatar(userList.get(position).getId_image());
//				UserUtils.setUserAvatar(getContext(), username, holder.avatar);
			}else {
				AppContext.toLog(userList.get(position).getId_image());
				UserUtils.setUserAvatar(getContext(), username, holder.avatar);
			}
