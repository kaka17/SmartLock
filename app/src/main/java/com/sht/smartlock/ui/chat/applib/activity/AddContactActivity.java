	...
	public void searchContact(View v) {
		...
		if (getString(R.string.button_search).equals(saveText)) {
			...
			if(TextUtils.isEmpty(name)) {
				...
				startActivity(new Intent(this, MyAlertDialog.class).putExtra("msg", st));
				return;
			}
...
//					String s = getResources().getString(R.string.Add_a_friend);
					EMContactManager.getInstance().addContact(emid, "aaa");
					runOnUiThread(new Runnable() {
						public void run() {
//							progressDialog.dismiss();
//							String s1 = getResources().getString(R.string.send_successful);
//							Toast.makeText(AddContactActivity.this, "hfghyfg", Toast.LENGTH_LONG).show();
							BaseApplication.toast(R.string.send_successful);
//							LogUtil.log("请求被发送，等待对等接受");
						}
					});
...
//							Toast.makeText(getApplicationContext(), s2 + e.getMessage(), Toast.LENGTH_LONG).show();
							BaseApplication.toast(R.string.Request_add_buddy_failure);
						}
					});
				}
			}
